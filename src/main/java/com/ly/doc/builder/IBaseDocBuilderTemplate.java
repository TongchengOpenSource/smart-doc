...
public interface IBaseDocBuilderTemplate {
     ...
     */
    default void bindingCommonVariable(ApiConfig config, JavaProjectBuilder javaProjectBuilder, Template template, boolean apiDocListEmpty) {
        String strTime = DateTimeUtil.long2Str(NOW, DateTimeUtil.DATE_FORMAT_SECOND);
    ...
    default void copyJQueryAndCss(ApiConfig config) {
        ...
        IBaseDocBuilderTemplate.copyJarFile("css/" + DocGlobalConstants.FONT_STYLE, config.getOutPath() + DocGlobalConstants.FILE_SEPARATOR + DocGlobalConstants.FONT_STYLE);
        IBaseDocBuilderTemplate.copyJarFile("js/" + DocGlobalConstants.JQUERY, config.getOutPath() + DocGlobalConstants.FILE_SEPARATOR + DocGlobalConstants.JQUERY);
        IBaseDocBuilderTemplate.copyJarFile("js/" + DocGlobalConstants.HIGH_LIGHT_JS, config.getOutPath() + DocGlobalConstants.FILE_SEPARATOR + DocGlobalConstants.HIGH_LIGHT_JS);
        IBaseDocBuilderTemplate.copyJarFile("css/" + DocGlobalConstants.FONT_STYLE, config.getOutPath() + DocGlobalConstants.FILE_SEPARATOR + DocGlobalConstants.FONT_STYLE);
    }

    /**
     * Copies a file from a source path to a target path.
     * This method is used to copy a file from a specified source path to a target path.
     * It is primarily used to copy the all-in-one stylesheet and other supporting files to the output directory.
     *
