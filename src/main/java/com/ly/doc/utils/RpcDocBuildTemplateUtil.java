...
public class RpcDocBuildTemplateUtil {

    public static List<RpcJavaMethod> buildServiceMethod(final JavaClass cls, ApiConfig apiConfig,
            ProjectDocConfigBuilder projectBuilder, RpcDocBuildTemplate rpcDocBuildTemplate) {
        ...
        for (JavaMethod method : methods) {
            ...
            if (needAllMethods || filterMethods.contains(method.getName())) {
                RpcJavaMethod apiMethodDoc = rpcDocBuildTemplate.convertToRpcJavaMethod(apiConfig, method, null);
                methodDocList.add(apiMethodDoc);
            }

        }
        // add parent class methods
        methodDocList.addAll(rpcDocBuildTemplate.getParentsClassMethods(apiConfig, cls));
        if (cls.isInterface() || cls.isAbstract()) {
            ...
            for (JavaType type : implClasses) {
                ...
                for (JavaMethod method : javaClass.getMethods()) {
                    if (!method.isDefault()) {
                        methodDocList
                                .add(rpcDocBuildTemplate.convertToRpcJavaMethod(apiConfig, method, actualTypesMap));
                    }
                }
            }
        }
        ...
        for (RpcJavaMethod method : methodDocList) {
            ...
            List<ApiParam> requestParams = requestParams(method.getJavaMethod(), projectBuilder,
                    new AtomicInteger(0), method.getActualTypesMap(), rpcDocBuildTemplate);
            // build response params
            List<ApiParam> responseParams = rpcDocBuildTemplate.buildReturnApiParams(
    ...
    public static List<ApiParam> requestParams(final JavaMethod javaMethod,
            ProjectDocConfigBuilder builder,
            AtomicInteger atomicInteger,
            Map<String, JavaType> actualTypesMap, RpcDocBuildTemplate rpcDocBuildTemplate) {
        ...
        for (JavaParameter parameter : parameterList) {
            ...
            String typeName = rpcDocBuildTemplate.replaceTypeName(parameter.getType().getGenericCanonicalName(),
                    actualTypesMap,
                    Boolean.FALSE);
            String simpleName = rpcDocBuildTemplate
                    .replaceTypeName(parameter.getType().getValue(), actualTypesMap, Boolean.FALSE)
                    .toLowerCase();
            String fullTypeName = rpcDocBuildTemplate.replaceTypeName(parameter.getType().getFullyQualifiedName(),
                    actualTypesMap,
                    Boolean.FALSE);
            ...
            StringBuilder comment = new StringBuilder(
                    rpcDocBuildTemplate.paramCommentResolve(paramTagMap.get(paramName)));
    ...
    public static boolean isEntryPoint(JavaClass cls, FrameworkAnnotations frameworkAnnotations) {
