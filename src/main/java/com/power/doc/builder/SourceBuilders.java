...
public class SourceBuilders {
    ...
    private static final String DEFAULT_SERVER_URL = "{server}";
     ...
     */
    public SourceBuilders(boolean isStrict, JavaProjectBuilder projectBuilder) {
     ...
     * @param projectBuilder JavaProjectBuilder
     */
    public SourceBuilders(ApiConfig config, JavaProjectBuilder projectBuilder) {
        if (null == config) {
...
//                }
                ...
                // build request json
                HashMap<String,String> requestJson = buildReqJsonOld(method, apiMethodDoc, isPostMethod);

                //build request urlParams json
                apiMethodDoc.setRequestUrlParam(requestJson.get(JSON_GET_PARAMS ));
                //build request body json
                apiMethodDoc.setRequestBody(requestJson.get(JSON_REQUEST_BODY));
...
//                apiMethodDoc.setRequestUsage(requestJson);
     ...
     */
    public  HashMap<String,String> buildReqJsonOld(JavaMethod method, ApiMethodDoc apiMethodDoc, Boolean isPostMethod) {
        //post请求体
        String requestBody;
        //Url 参数
        String urlParams;
        ...
        if (isPostMethod) {
                ...
                for (JavaAnnotation annotation : parameter.getAnnotations()) {
                    ...
                    if (DocGlobalConstants.SHORT_REQUEST_BODY.equals(annotationName) || DocGlobalConstants.REQUEST_BODY_FULLY.equals(annotationName)) {
                        ...
                        if (DocClassUtil.isPrimitive(simpleTypeName)) {
                            ...
                            requestBody  =buildJson(typeName, gicTypeName, this.fieldMap, false, 0, new HashMap<>());
                            requestParams.put(JSON_REQUEST_BODY,JsonFormatUtil.formatJson(requestBody));
                            return requestParams;
                        }
                    }
                }
            ...
            for (JavaParameter parameter : parameterList) {
                ...
                formData.setValue(buildFormDataParam(parameter));
     ...
     * @return formData 数据
     */
    private  String buildFormDataParam(JavaParameter javaParameter ) {
        ...
        //是一个复杂对象
            ...
            String data = buildJson(typeName, gicTypeName, this.fieldMap, false, 0, new HashMap<>());
