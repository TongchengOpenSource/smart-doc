...
/**
 * @author xingzi  2019/12/21  21:29
 */
...
//                }
    ...
    private String buildReqJson(JavaMethod method, ApiMethodDoc apiMethodDoc, Boolean isPostMethod) {
        ...
        //post请求
        if (isPostMethod) {
            //不处理@RequestParam 和 @PathVariable
//            Map<String, String> paramMap = ReqJsonUtil.buildGetParam(parameterList);
//            String reqUrl = ReqJsonUtil.buildUrl(apiMethodDoc.getUrl(), false, paramMap);
//            apiMethodDoc.setUrl(reqUrl);
            ...
            return JsonFormatUtil.formatJson(new Gson().toJson(formDataList));
        }
        //get请求
        else {
            boolean containsBrace = apiMethodDoc.getUrl().contains("{");
            Map<String, String> paramsMap = ReqJsonUtil.buildGetParam(parameterList);
            //构建Get参数
            return ReqJsonUtil.buildUrl(apiMethodDoc.getUrl(), containsBrace, paramsMap);
        }
    }
     ...
     * @return
     */
    public String buildFormDataParam(JavaParameter javaParameter) {
